...
#-------------------------------------------------------------------------------------------------------------
#
# Syntax: ./common-arch.sh [install zsh flag] [username] [user UID] [user GID] [upgrade packages flag] [install Oh My Zsh! flag] 
...
if [ "${PACKAGES_ALREADY_INSTALLED}" != "true" ]; then
    ...
    if rpm -qi libopenssl1_1 >/dev/null 2>&1; then
        package_list="${package_list} libopenssl1_1"
    fi
    
    echo "Packages to verify are installed: ${package_list}"
    zypper in -y ${package_list}
